...
if ($resultado5==''){
 ...
 $miinput = new input('radio','id="respuesta'.$resultado4.'" name="respuesta'.$resultado4.'" class=""','','');
 $miinput->crearInput();
...
if ($resultado=='1'){
	...
	if ($resultado5==''){
	echo '<div class="radiorespuesta'.$posicion.'" style="margin:5px;">';
	$miinput = new input('radio','id="respuestaradio'.$resultado4.'" class="radiocrear" name="respuesta_radio_'.$resultado4.'" value="'.$resultado4.'"','','');
	$miinput->crearInput();
	echo '<div style="float:left;line-height:2;max-width:93%" class="txt13gris">'.$resultado3.'</div>'; 
	
	}
else  if ($resultado5!=''){
	echo '<div class="radiorespuesta'.$posicion.'" style="width:25%;">';
	
		echo '<div style="max-width:60%" class="imagenradio'.$posicion.'"><img src="'.$GLOBALS['raiz'].'content/answers/'.$resultado5.'"></div>';
